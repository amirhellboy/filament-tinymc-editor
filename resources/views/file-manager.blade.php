...
<head>
    ...
    <style>
        ...
        .card { border: 0; border-radius: 8px; padding: 0; text-align: center; cursor: pointer; }
        .thumb { width: 100%; height: 160px; object-fit: cover; border-radius: 6px; background: #f3f4f6; display:flex; align-items:center; justify-content:center; }
        .name { font-size: 14px; margin-top: 10px; color: #111827; word-break: break-word; }
        .empty { color: #6b7280; text-align: center; margin-top: 24px; }
        ...
        .upload-label { border:1px solid #e5e7eb; background:#fff; color:#111827; padding:8px 12px; border-radius:6px; cursor:pointer; }
        .breadcrumbs { display:flex; gap:6px; align-items:center; margin-bottom: 12px; flex-wrap: wrap; }
        .crumb { color:#2563eb; text-decoration: none; cursor:pointer; }
        .crumb-sep { color:#6b7280; }
        .row-actions { margin-top:6px; display:flex; gap:6px; justify-content:center; }
        .muted { color:#6b7280; font-size:12px; }
        .bar { display:flex; align-items:center; gap:8px; margin-bottom: 12px; }
        .search { border:1px solid #e5e7eb; border-radius:6px; padding:8px 10px; width: 260px; }
        .table { width: 100%; border-collapse: collapse; }
        .table th, .table td { padding: 10px; text-align: left; }
        .right { text-align: right; }
        .table tr { border-bottom: 1px solid #e5e7eb; }
        .view-toggle { border:1px solid #e5e7eb; padding:6px 10px; border-radius:6px; background:#fff; color:#111827; }
        .plus-btn { width:36px; height:36px; border-radius:8px; border:none; background:#2563eb; color:#fff; font-size:22px; display:flex; align-items:center; justify-content:center; }
        .dropdown { position:relative; }
        .menu { display:none; position:absolute; right:0; top:42px; background:#fff; border:1px solid #e5e7eb; border-radius:8px; min-width:200px; box-shadow:0 10px 20px rgba(0,0,0,.08); z-index:20; overflow:hidden; }
        .menu button { width:100%; text-align:left; padding:10px 12px; background:#fff; border:none; display:flex; gap:8px; align-items:center; color:#111827; }
        .menu button:hover { background:#eef2ff; }
        .selection-bar { display:none; position:sticky; top:0; z-index:15; background:#111827; color:#fff; border-bottom:1px solid #111827; padding:10px 12px; margin:-16px; margin-bottom:12px; align-items:center; justify-content:space-between; }
        .selection-left { display:flex; align-items:center; gap:12px; font-weight:600; color:#fff; }
        .icon-btn { background:none; border:none; font-size:20px; cursor:pointer; }
        .row-selected { background:#111827; color:#fff; }
...
<div id="selectionBar" class="selection-bar">
    ...
    <div style="display:flex; align-items:center; gap:10px;">
        <button class="view-toggle" style="background:#dc2626;color:#fff;border-color:#b91c1c" onclick="bulkDelete()">Delete</button>
        <button class="view-toggle" onclick="unselectAll()">Unselect All</button>
    </div>
</div>
<div class="breadcrumbs">
    <a class="crumb" href="#" onclick="goTo('')">uploads</a>
...
<script>
    ...
    async function renameItem(path, isDir) {
        ...
        const res = await fetch('{{ route('tinymc-editor.rename') }}', {
            method: 'POST',
        ...
        if (isDir) {
            ...
            if (d) { d.name = updatedName; d.path = updatedPath; }
        } else {
            const f = (initialFiles || []).find(x => normalizePath(x.path) === normPath);
            if (f) { f.name = updatedName; f.path = updatedPath; }
        }
    ...
    async function removeItem(path) {
        ...
        try {
            ...
            const url = new URL('{{ route('tinymc-editor.delete') }}');
        ...
        if (view === 'list') {
            ...
            data.forEach(f => {
                ...
                const icon = f._type === 'dir'
                    ...
                    : (isImg ? `<img src="${f.url}" style="width:28px;height:28px;object-fit:cover;border-radius:4px;margin-right:8px;">` : `<span style="display:inline-flex;width:28px;height:28px;border:1px solid #e5e7eb;border-radius:4px;align-items:center;justify-content:center;margin-right:8px;">📄</span>`);
                const mid = base64Id(f.path);
                tr.innerHTML = `
                        <td><input type="checkbox" data-path="${f.path}" ${selected.has(f.path) ? 'checked' : ''}></td>
                        <td style="display:flex;align-items:center;">${icon}<span style="cursor:pointer;color:#111827;" onclick="${f._type === 'dir' ? `goTo('${f.path}')` : `selectFile('${f.url.replace(/'/g, '')}')`}" >${f.name}</span></td>
            ...
            data.forEach(f => {
                ...
                card.className = 'card';
                card.onclick = () => f._type === 'dir' ? goTo(f.path) : selectFile(f.url);
                const isImg = f._type === 'file' && imageExts.includes(f.ext);
                const content = f._type === 'dir'
                    ...
                    : (isImg ? `<img class="thumb" src="${f.url}">` : `<div class="thumb"><span style="display:inline-flex;width:64px;height:64px;border:1px solid #e5e7eb;border-radius:8px;align-items:center;justify-content:center;">📄</span></div>`);
                card.innerHTML = `
                        ${content}
    ...
    function unselectAll() { selected.clear(); updateSelectionUI(); render(); }
    ...
    function hideAddMenu() { const menu = document.getElementById('addMenu'); if (menu) menu.style.display = 'none'; }
    ...
    function promptCreateFolder() {
        ...
        if (!name) return;
        fetch('{{ route('tinymc-editor.folder') }}', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content },
            credentials: 'same-origin',
    ...
    if (picker) {
        picker.addEventListener('change', async (e) => {
            ...
            try {
                const res = await fetch('{{ route('tinymc-editor.upload') }}', {
                    method: 'POST',
                    headers: {
                        'X-Requested-With': 'XMLHttpRequest',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
